...
@if(count($posts) == 0)
@section('title', 'Sin informacion')
@else
    @section('title', 'Publicaciones de: '. $posts->first()->email)
@endif
@section('content')
...
@if(count($posts) == 0)
    <div class="panel shadow mtop16">
        <h2 class="title">No existe informacion</h2>
    </div>
@else
                        ...
                        <div class="inside">
                            <div class="mini_profile">
                                <div class="info">
                                    ...
                                    <strong><span class="text">{{ $posts->first()->user_name }}</span></strong>
                                    <span class="title"><i class="far fa-envelope"></i> Email:</span>
                                    <strong><span class="text">{{ $posts->first()->email }}</span></strong>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="inside">
                            <div class="mini_profile">
                                <div class="venues">
                                    ...
                                    <strong><span class="text">{{ $posts->first()->venue_name }}</span></strong>
                                    <span class="title"><i class="fas fa-graduation-cap"></i> Carrera:</span>
                                    <strong><span class="text">{{ $posts->first()->career_name }}</span></strong>
                                    <span class="title"><i class="fas fa-clipboard-check"></i> Semestre:</span>
                                    <strong><span class="text">{{ $posts->first()->semester_name }}</span></strong>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-body py-3">
                        <div class="row no-gutters align-items-center">
                            <div class="col"> <a href="{{ url('/post/'.$post->id.'/'.$post->slug) }}" style="color: rgb(208, 0, 0); text-decoration: none" class="text-big" data-abc="true">{{ substr($post->details,0,50) }}...</a>
                                <div class="text-muted small mt-1">{{ date_format($post->created_at,'d/m/Y') }} &nbsp;·&nbsp; <a href="javascript:void(0)" class="text-muted" data-abc="true" style="text-decoration: none; cursor:auto;">{{ $posts->first()->email }}</a></div>
